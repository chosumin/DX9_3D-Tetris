...
cMouse::cMouse()
{
	...
	ZeroMemory(buttonStatus, sizeof(byte) * MAX_INPUT_cMouse);
	ZeroMemory(buttonOldStatus, sizeof(byte) * MAX_INPUT_cMouse);
	ZeroMemory(buttonMap, sizeof(byte) * MAX_INPUT_cMouse);

	ZeroMemory(startDblClk, sizeof(DWORD) * MAX_INPUT_cMouse);
	ZeroMemory(buttonCount, sizeof(int) * MAX_INPUT_cMouse);

	timeDblClk = GetDoubleClickTime();
	startDblClk[0] = GetTickCount();

	for ( int i = 1; i < MAX_INPUT_cMouse; i++ )
...
void cMouse::Update()
{
	...
	for ( DWORD i = 0; i < MAX_INPUT_cMouse; i++ )
	{
...
LRESULT cMouse::InputProc(UINT message, WPARAM wParam, LPARAM lParam)
{
